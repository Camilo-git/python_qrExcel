import os  # Utilidades de sistema de archivos y rutas
import re  # Expresiones regulares para sanear nombres de archivos
import sys  # Detección de modo congelado (PyInstaller) y errores a stderr
...
try:
    ...
    # Sugerencia útil para Linux si falta Tkinter
    print(
...
from openpyxl import load_workbook  # Lectura de Excel
from openpyxl.utils import get_column_letter  # Conversión 1->A, 2->B, ...
...
class ExcelPreviewApp(tk.Tk):
    ...
    Funcionalidad:
    - Cargar un Excel (.xlsx/.xlsm).
    - Previsualizar columnas A y B (todas las filas) en una tabla.
    - Generar imágenes PNG de QR usando la columna B (texto/URL) y nombrar con la columna A.
    """
    ...
    def _build_ui(self) -> None:
        ...
        self.qr_btn = ttk.Button(top, text="Generar QR", command=self.on_generate_qr)  # Crea PNGs
        ...
        # Tabla con scrollbars para previsualizar A y B
        ...
        # Barra de estado con mensajes al usuario
        status = ttk.Label(self, textvariable=self.status_var, anchor="w")
        status.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=(0, 8))

    def clear_table(self) -> None:
        """Limpia columnas y filas del Treeview, y actualiza estado."""
        if not self.tree:
            return
    ...
    def on_load_excel(self) -> None:
        """Diálogo para seleccionar Excel y cargarlo en la tabla."""
        ...
        if not path:
            return
        try:
            rows = self.read_two_columns_all_rows(path)  # Lee solo A y B de la hoja activa
        except Exception as e:
    ...
    def read_first_two_rows(self, path: str) -> List[List[Optional[object]]]:
        """Utilidad de depuración: devuelve las 2 primeras filas completas de la hoja activa."""
    ...
    def read_two_columns_all_rows(self, path: str) -> List[List[Optional[object]]]:
        """Lee todas las filas de la hoja activa, únicamente columnas A (título) y B (URL/texto)."""
        wb = load_workbook(path, read_only=True, data_only=True)
        ws = wb.active
        if ws is None:
            wb.close()
        ...
        for row in ws.iter_rows(values_only=True):
            if row is None:
                ...
                continue
            a = row[0] if len(row) > 0 else None  # Columna A
            b = row[1] if len(row) > 1 else None  # Columna B
            out.append([a, b])
        wb.close()
        return out

    def populate_table(self, rows: List[List[Optional[object]]]) -> None:
        """Configura columnas A y B en el Treeview y vuelca todas las filas."""
        if not self.tree:
            return
        self.clear_table()
        ...
        if max_cols == 0:
            ...
            return
        columns = [get_column_letter(i + 1) for i in range(max_cols)]  # ['A', 'B']
        self.tree["columns"] = columns
        for col in columns:
            self.tree.heading(col, text=col)
            self.tree.column(col, width=140, anchor="center")  # ancho fijo para legibilidad
        for r in rows:
            # Normaliza None -> "" y asegura longitud de fila == max_cols
    ...
    def on_generate_qr(self) -> None:
        """Genera imágenes PNG de QR a partir de las columnas A (nombre) y B (contenido)."""
    ...
    def _safe_filename(self, name: str) -> str:
        """Devuelve un nombre de archivo seguro (sin caracteres problemáticos)."""
        name = name.strip()
        name = re.sub(r"[^\w\-\. ]+", "_", name)  # Sustituye todo lo que no sea letra/número/_/./-/espacio
        name = re.sub(r"\s+", "_", name)  # Espacios contiguos -> guion bajo
        return name or "qr"

    def generate_qr_images(self, path: str, skip_header: bool = False) -> Tuple[int, int]:
        # Determina la carpeta base de salida:
        # - Si está congelado (PyInstaller), usar la carpeta del .exe.
        # - Si no, usar la carpeta del script.
        if getattr(sys, "frozen", False):  # ejecutable PyInstaller
            base_dir = os.path.dirname(sys.executable)
        else:
            base_dir = os.path.dirname(os.path.abspath(__file__))
        out_dir = os.path.join(base_dir, "img")  # Carpeta donde se guardan los PNG
        ...
        for r_idx, row in enumerate(ws.iter_rows(values_only=True), start=1):
            if skip_header and r_idx == 1:  # Saltar encabezado si corresponde
                continue
            title = row[0] if row and len(row) > 0 else None  # Columna A
            url = row[1] if row and len(row) > 1 else None    # Columna B
            ...
            fname_base = self._safe_filename(title_str)  # Nombre de archivo a partir de A
            dest = os.path.join(out_dir, f"{fname_base}.png")  # Ruta destino PNG
            suffix = 1
            while os.path.exists(dest):  # Evita sobrescribir: agrega sufijo incremental
                dest = os.path.join(out_dir, f"{fname_base}_{suffix}.png")
                suffix += 1

            # Construcción del QR (versión pequeña, borde estándar)
...
if __name__ == "__main__":
    app = ExcelPreviewApp()  # Crea e inicia la app
    app.mainloop()  # Bucle principal de la interfaz
